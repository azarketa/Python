...
import TDMS_Packages.MathTools as mtools
...
class TdmsFileRoot:
    ...
    def __wake_rake_init__(self, file_path, file_alias, ref_mag):
        ...
        # undertake the data loading part.
        self.__means_only_init__(file_path, file_alias, ref_mag)
        # Calls the internal __runs_counter__ method for performing the attribute structuring task.
    ...
    def __set_standard_data__(self):
        ...
        # Calls the internal __non_dimensionalize__ method with the correspondent refMag object (referential
        # magnitudes) and the nonDimensaionlizeGroups.original flag indicating that just the original group is to be
        # non-dimensionalized.
    ...
    def __set_means__(self):
        ...
        if boolean_flag:
            ...
            for group in [group for group in self.__file__.groups() if group not in ["Initial drift", "Final drift"]]:
                ...
                for pos_prop in [prop for prop in dir(self.groups_original.__getattribute__(group)) if (prop.__repr__() in ["'x_pos'", "'y_pos'", "'z_pos'", "'angle_pos'"])]:
                    ...
                    if hasattr(self.groups_added.means_group.means_channels, pos_prop):
                        self.groups_added.means_group.means_channels.__getattribute__(pos_prop).append(self.groups_original.__getattribute__(group).__getattribute__(pos_prop))
                    else:
                        self.groups_added.means_group.means_channels.__setattr__(pos_prop, [self.groups_original.__getattribute__(group).__getattribute__(pos_prop)])
                # A channel level "for" loop that runs over the channels of the group.
                # for channel in self.__file__.group_channels(group):
                for channel in [not_dunder_property for not_dunder_property in dir(self.groups_original.__getattribute__(group).channels) if ('__' not in not_dunder_property.__repr__())]:
                    # Conditional that checks whether the groups_added variable already contains a child variable
    ...
    def __set_wake_rake__(self, wake_rake_port_ordering=tuple((np.arange(1, 19))), wake_rake_cross_axis=TDMSE.deviceAxis.y):
        ...
        wake rake probes.
        :param wake_rake_cross_axis: this is a TDMSEnum custon enum calss providing information of the main wind tunnel axis that has
        driven the experiment. As it is considered that, usually, the wake rake is positioned with its probes' lines aligned with the y-axis
        ...
        parameter by which to sort the data.
        ...
        eval_string, runs_info = self.__runs_counter__(runs_per_param_mode = TDMSE.runsPerParamMode.wake_rake, device_axis=wake_rake_cross_axis)
        ...
        # Declaring the 'wake_rake_value_list' as an empty list. This list is intended to store the multiple wake rake values.
            ...
            for _ in np.arange(parent_param_index * i, parent_param_index * (i + 1)):
                ...
                for actualIndex in range(0, actual_param_index):
                    ...
                    for port in wake_rake_port_ordering:
                        wake_rake_pos.append(eval(eval_string + ".__getattribute__(" + (actual_param + "_pos").__repr__() + ")")[actualIndex] + (-wake_rake_port_ordering.index(port) + 9) * 2.505 / (1000 * self.__ref_mag__.length) - 1.25 / (1000 * self.__ref_mag__.length))
                        if wake_rake_port_ordering.index(port) == 12:
                            if 0.5 * (self.groups_added.means_group.means_channels.__getattribute__("port_" + str(wake_rake_port_ordering[11]) + "_signal")[specific_index] +
                                      self.groups_added.means_group.means_channels.__getattribute__("port_" + str(wake_rake_port_ordering[13]) + "_signal")[
                                          specific_index]) > 1.0:
                            ...
                            if self.groups_added.means_group.means_channels.__getattribute__("port_" + str(port) + "_signal")[specific_index] > 1.0:
                                wake_rake_values.append(1.0)
                            else:
        ...
        integrate_integrate_x = [int_int_x for int_int_x in eval(eval_string + ".__getattribute__(" + (wake_rake_cross_axis.name + "_rake_pos").__repr__() + ")")]
        ...
        for integrate_pre_process in integration_pre_process:
            ...
            int_list = [np.sqrt(int_y) * (1.0 - np.sqrt(int_y)) - np.sqrt(actual_average) * (1.0 - np.sqrt(actual_average)) for int_y in eval(integrate_pre_process)]
            x, y = zip(*sorted(zip(integrate_integrate_x, int_list)))
            integrate_y = [int_y for int_y in y]

            cdf = mtools.get_cdf(integrate_y)
    ...
    # TODO: Implement internal '__set_kistler__()' method.
    def __set_kistler__(self, runs_per_param_mode, device_axis=TDMSE.deviceAxis.angle):
        raise NotImplementedError

    # Internal __runs_counter__ method.
    def __runs_counter__(self, runs_per_param_mode, device_axis=TDMSE.deviceAxis.y):
        ...
        if (np.nanprod([runs_per_param[0] for runs_per_param in sorted_runs_info]) == len(self.groups_added.means_group.means_channels.z_pos)):            
    ...
    def __non_dimensionalize__(self, file_obj, ref_mag):
        ...
        for item in file_obj.objects.items():
            ...
            item_name_list = item[0].lower().replace("/", "").replace("''", '"').replace("'", "").split('"')
            if (len(item_name_list) == 1) and (item_name_list[0] != ""):
                if hasattr(self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).__getattribute__("channels"), "thermocouple_signal"):
                    ref_mag.temp = np.average(self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).__getattribute__("channels").__getattribute__("thermocouple_signal").data) + 273
                if hasattr(self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).__getattribute__("channels"), "fix_probe_signal"):
                    ref_mag.u = np.average(self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).__getattribute__("channels").__getattribute__("fix_probe_signal").data)
                self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).x_pos /= (1000 * ref_mag.length)
                self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).y_pos /= (1000 * ref_mag.length)
                self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).z_pos /= (1000 * ref_mag.length)
            if "unit_string" in item[1].properties:
                if item[1].property("unit_string") == "Pa" and ref_mag.q:
                        self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).channels.__getattribute__(item_name_list[1].replace(" ", "_")).data /= ref_mag.q
                elif item[1].property("unit_string") == "N" and ref_mag.q:
                    self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).channels.__getattribute__(item_name_list[1].replace(" ", "_")).data /= (ref_mag.q * ref_mag.length * ref_mag.span)
                elif item[1].property("unit_string") == "Nm" and ref_mag.q:
                    self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).channels.__getattribute__(item_name_list[1].replace(" ", "_")).data /= (ref_mag.q * (ref_mag.length ** 2.0) * ref_mag.span)
                elif item[1].property("unit_string") == "m/s":
                    self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).channels.__getattribute__(item_name_list[1].replace(" ", "_")).data /= ref_mag.u
                elif item[1].property("unit_string") == "ºC":
                    self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).channels.__getattribute__(item_name_list[1].replace(" ", "_")).data += 273
                    self.groups_original.__getattribute__(item_name_list[0].replace(" ", "_")).channels.__getattribute__(item_name_list[1].replace(" ", "_")).data /= ref_mag.temp
                else:
                    pass
    ...
    def process_standard_data(self):
        ...
        self.__set_standard_data__()
    
    
    # Public method meansProcessing.
    def process_means(self):
    ...
    def process_wake_rake(self, wake_rake_port_ordering=tuple(np.arange(1, 19)), wake_rake_cross_axis=TDMSE.deviceAxis.y):
        ...
        # Calling the internal __runs_counter__ method for performing the task of attribute structuring.
        self.__set_wake_rake__(wake_rake_port_ordering = wake_rake_port_ordering, wake_rake_cross_axis = wake_rake_cross_axis)
        
    # TODO: implement public 'process_kistler()'
    def process_kistler(self):
        raise NotImplementedError
    ...
    def __init__(self, file_obj, group_objs=None, generate_isolated_groups=False):
        ...
        if not generate_isolated_groups:
            ...
            for groupObj in group_objs:
                self.__setattr__(groupObj.lower().replace(" ", "_"), __TdmsFileGroup__(nptdms.GroupObject(groupObj), group_index, file_obj, False))
                group_index += 1
        else:
            pass
    ...
    def __init__(self, group_obj, group_index, file_obj, generate_isolated_group=False):
        ...
        if not generate_isolated_group:
            ...
            if file_obj is None:
                ...
                self.channels = __TdmsGroupChannels__(self.group, file_obj.group_channels(self.group_name), generate_isolated_channels=False)
        else:
            pass
    ...
    def __init__(self, group_obj, channel_objs, generate_isolated_channels=False):
        ...
        if not generate_isolated_channels:
            ...
            for channel_obj in channel_objs:                
                self.__setattr__(channel_obj.channel.lower().replace(" ", "_"), __TdmsGroupChannel__(channel_obj.channel.lower().replace(" ", "_"), nptdms.ChannelObject(channel_obj, group_obj, channel_obj.data)))
        else:
            pass
...
class RefMagnitudes:
    ...
    def __post_init__(self):
        # Density computation by perfect gas law.
        self.__rho__ = self.p / (self.r_const * self.temp)
        # Viscosity computation by Sutherland's law.
        self.__mu__ = (1.456e-6 * self.temp ** (3. / 2.)) / (self.temp + 110.4)          
    ...
    for file in list_of_files:        
        if type(file) == TdmsFileData:
            ...
            if reload or 'p' not in parent_path_file_extensions:
                caller_frame.f_globals[file_alias_no_whitespace] = TdmsFileRoot(file.file_path, file.file_read_mode, file.file_alias, file.file_ref_mag)
